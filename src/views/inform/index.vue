    ...
    <div class="container newBox" :class="{ hContainer: baseData.length }">
      <div class="informList" v-if="baseData.length > 0">
        <div v-for="(item, index) in baseData" :key="index">
          ...
          >
            <div :class="isActiven ? 'titAlready' : ''">
              <div class="tit">
...
export default class extends Vue {
  ...
  private isActiven = false

  // Explicitly declare ountUnread as a getter so TypeScript recognizes it
  get ountUnread(): number {
    return AppModule.statusNumber
  }

  get tabList(): Array<{ label: string; value: number }> {
    return [
      {
        label: '未读',
        value: 1,
  ...
  // 设置单个订单已读
  async handleSetStatus(id:number) {
  ...
  // 下拉菜单显示
  toggleShow(id: number, index: number) {
    this.shopShow = true
