...
export default class extends Vue {
  ...
  //接单
  orderAccept(row: any, event: Event) {
  ...
  //打开取消订单弹窗
  cancelOrder(row: any, event: Event) {
    event.stopPropagation()
  ...
  //打开拒单弹窗
  orderReject(row: any, event: Event) {
    event.stopPropagation()
    this.cancelDialogVisible = true
    this.orderId = row.id
    this.dialogOrderStatus = row.status
  ...
  // 派送，完成
  cancelOrDeliveryOrComplete(status: number, id: string, event: Event) {
    event.stopPropagation()
    const params = {
      status,
      id,
  ...
  // 查看详情
  async goDetail(id: any, status: number, row: any, event: Event) {
    event.stopPropagation()
